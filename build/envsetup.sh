function __print_ozone_functions_help() {
cat <<EOF
Additional Ozone functions:
...
function breakfast()
{
    target=$1
    local variant=$2
    OZONE_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/ozone/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Ozone model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch ozone_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ozone-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.ozone.device | grep -q "$OZONE_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $OZONE_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.ozone.device | grep -q "$OZONE_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        ozoneremote
        git push ozone HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.ozone.device | grep -q "$OZONE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/ozone/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $OZONE_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/ozone/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
