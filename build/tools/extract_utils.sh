...
function setup_vendor() {
    ...
    export OZONE_ROOT="$3"
    if [ ! -d "$OZONE_ROOT" ]; then
        echo "\$OZONE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$OZONE_ROOT/$OUTDIR" ]; then
        mkdir -p "$OZONE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$OZONE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$OZONE_ROOT"/"$OUTDIR"/Android.mk
    export ANDROIDBP="$OZONE_ROOT"/"$OUTDIR"/Android.bp
    export BOARDMK="$OZONE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "$COMMENT_PREFIX Copyright (C) $YEAR The Ozone Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017-$YEAR The Ozone Project\n" >> $1
        else
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The Ozone Project\n" >> $1
        fi
    else
        printf "$COMMENT_PREFIX Copyright (C) $YEAR The Ozone Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OZONE_ROOT"/vendor/ozone/build/tools/smali/baksmali.jar
        export SMALIJAR="$OZONE_ROOT"/vendor/ozone/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$OZONE_ROOT"/vendor/ozone/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$OZONE_ROOT"/vendor/ozone/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$OZONE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$OZONE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$OZONE_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$OZONE_ROOT"/vendor/ozone/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$OZONE_ROOT"/"$OUTDIR"/radio
