...
function setup_vendor() {
    ...
    if [ ! -d "$LUCID_ROOT" ]; then
        echo "\$LUCID_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$LUCID_ROOT/$OUTDIR" ]; then
        mkdir -p "$LUCID_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$LUCID_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$LUCID_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$LUCID_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$LUCID_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR Lucid Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR Lucid Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR Lucid Project\n" >> $1
    fi
...
function oat2dex() {
    ...
    if [ ! -f "$LUCID_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$LUCID_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$LUCID_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$LUCID_ROOT"/vendor/lucid/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$LUCID_ROOT"/"$OUTDIR"/radio
