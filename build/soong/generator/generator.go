...
func init() {
	android.RegisterModuleType("fi_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fiExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fiExpandVariables(ctx, dep_file)
	...
	cmd := fiExpandVariables(ctx, String(g.properties.Cmd))
