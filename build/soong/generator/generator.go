...
func init() {
	android.RegisterModuleType("ozone_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ozoneExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ozoneExpandVariables(ctx, dep_file)
	...
	cmd := ozoneExpandVariables(ctx, String(g.properties.Cmd))
